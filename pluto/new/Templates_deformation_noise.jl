...
# ╔═╡ f7f1f2de-af31-4bd3-9e23-f6bc55c4e8d8
savefig(plot(Gray.(J₁), axis=false, grid=false), "org_img.pdf")
...
savefig(plot(Gray.(T), axis=false, grid=false), "dis_img.pdf")
	...
	# for j in 1:n
		for k in 1:m
			# for l in 1:m
				...
				c = mk_temp_dis[k][1]
				d = mk_temp_dis[k][2]
				C[i, k] = abs(a - c)^np + abs(b - d)^np
				# C[i, k] = max(abs(a - c), abs(b - d))
			# end
		end
	# end
end
...
# ╔═╡ 78176a36-2eeb-476e-ab23-f849ae2ad3b3
heatmap(P)
