...
# ╔═╡ 886a00cf-5562-4dfe-ac50-3e43c286f97b
iNRCDT = MinNormRadonCDT(RCDT)

# ╔═╡ 553a0f34-84b4-4997-b00d-c90fdb1ae833
mqClass = mNRCDT.(TClass); iqClass = iNRCDT.(TClass);

# ╔═╡ 6f89e4df-7af6-4fdd-bf50-029b07ca82c2
mqTemp = mNRCDT.(J); iqTemp = iNRCDT.(J);
...
# ╔═╡ 1daf3cc1-5fa4-412d-adcf-04b37939c299
ip = plot_quantiles(iqTemp, Label, iqClass, Labels)
...
for angle in [2,4,8,16,32,64,128,256]
	...
	RCDT = RadonCDT(64, R)
	rcdtClass = RCDT.(TClass)
	rcdtTemp = RCDT.(J)
	#mNRCDT = MaxNormRadonCDT(RCDT)
	#mqClass = mNRCDT.(TClass)
	#mqTemp = mNRCDT.(J)
	mqClass = max_normalization.(rcdtClass)
	mqTemp = max_normalization.(rcdtTemp)
	iqClass = min_normalization.(rcdtClass)
	iqTemp = min_normalization.(rcdtTemp)
	...
	accuracy_nearest_neighbour(iqTemp, Label, iqClass, Labels, "inf", ret=1)
	accuracy_nearest_neighbour(iqTemp, Label, iqClass, Labels, "euclidean", ret=1)
end
