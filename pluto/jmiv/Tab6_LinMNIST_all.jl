...
# ╔═╡ 246b05be-4cf9-4254-84f6-d8523220cc1d
# ╠═╡ disabled = true
#=╠═╡
for prop in [11,25,50]
	for KK in [1,5,11]
		@info "split" prop, "k-NN" KK
		Random.seed!(42); CC = accuracy_k_nearest_part_neighbour(20, prop, 500, 10, mqClass, MLLabel, "inf", K=KK, ret=1);
		jldsave("conf_LinMNIST_$(KK)NN_$(prop)_maxNRCDT_inf.jld2"; CC)
		Random.seed!(42); CC = accuracy_k_nearest_part_neighbour(20, prop, 500, 10, mqClass, MLLabel, "euclidean", K=KK, ret=1);
		jldsave("conf_LinMNIST_$(KK)NN_$(prop)_maxNRCDT_eucl.jld2"; CC)
		Random.seed!(42); CC = accuracy_k_nearest_part_neighbour(20, prop, 500, 10, miqClass, MLLabel, "inf", K=KK, ret=1);
		jldsave("conf_LinMNIST_$(KK)NN_$(prop)_maxminRCDT_inf.jld2"; CC)
		Random.seed!(42); CC = accuracy_k_nearest_part_neighbour(20, prop, 500, 10, miqClass, MLLabel, "euclidean", K=KK, ret=1);
		jldsave("conf_LinMNIST_$(KK)NN_$(prop)_maxminRCDT_eucl.jld2"; CC)
		Random.seed!(42); CC = accuracy_k_nearest_part_neighbour(20, prop, 500, 10, tvqClass, MLLabel, "inf", K=KK, ret=1);
		jldsave("conf_LinMNIST_$(KK)NN_$(prop)_tvNRCDT_inf.jld2"; CC)
		Random.seed!(42); CC = accuracy_k_nearest_part_neighbour(20, prop, 500, 10, tvqClass, MLLabel, "euclidean", K=KK, ret=1);
		jldsave("conf_LinMNIST_$(KK)NN_$(prop)_tvNRCDT_eucl.jld2"; CC)
	end
end
  ╠═╡ =#

# ╔═╡ a15b7097-df99-4cb8-9d85-9de9cab9cf63
# ╠═╡ disabled = true
#=╠═╡
for prop in [11,25,50]
	for KK in [1,5,11]
		@info "split" prop, "k-NN" KK
		Random.seed!(42); CC = accuracy_k_nearest_part_neighbour(20, prop, 500, 10, maqClass, MLLabel, "inf", K=KK, ret=1);
		jldsave("conf_LinMNIST_$(KK)NN_$(prop)_maxabsRCDT_inf.jld2"; CC)
		Random.seed!(42); CC = accuracy_k_nearest_part_neighbour(20, prop, 500, 10, maqClass, MLLabel, "euclidean", K=KK, ret=1);
		jldsave("conf_LinMNIST_$(KK)NN_$(prop)_maxabsRCDT_eucl.jld2"; CC)
		Random.seed!(42); CC = accuracy_k_nearest_part_neighbour(20, prop, 500, 10, iaqClass, MLLabel, "inf", K=KK, ret=1);
		jldsave("conf_LinMNIST_$(KK)NN_$(prop)_minabsRCDT_inf.jld2"; CC)
		Random.seed!(42); CC = accuracy_k_nearest_part_neighbour(20, prop, 500, 10, iaqClass, MLLabel, "euclidean", K=KK, ret=1);
		jldsave("conf_LinMNIST_$(KK)NN_$(prop)_minabsRCDT_eucl.jld2"; CC)
		Random.seed!(42); CC = accuracy_k_nearest_part_neighbour(20, prop, 500, 10, miaqClass, MLLabel, "inf", K=KK, ret=1);
		jldsave("conf_LinMNIST_$(KK)NN_$(prop)_maxminabsRCDT_inf.jld2"; CC)
		Random.seed!(42); CC = accuracy_k_nearest_part_neighbour(20, prop, 500, 10, miaqClass, MLLabel, "euclidean", K=KK, ret=1);
		jldsave("conf_LinMNIST_$(KK)NN_$(prop)_maxminabsRCDT_eucl.jld2"; CC)
	end
end
  ╠═╡ =#
...
md"
- using the Euclidean embedding and RCDT embedding.
"
...
for prop in [11,25,50]
	for KK in [1,5,11]
		@info "split" prop, "k-NN" KK
		Random.seed!(42); CC= accuracy_k_nearest_part_neighbour(20, prop, 500, 10, Array{Float64}.(TMLClass), MLLabel, "inf", K=KK, ret=1);
		jldsave("conf_LinMNIST_$(KK)NN_$(prop)_eucl_inf.jld2"; CC)
		Random.seed!(42); CC= accuracy_k_nearest_part_neighbour(20, prop, 500, 10, Array{Float64}.(TMLClass), MLLabel, "euclidean", K=KK, ret=1);
		jldsave("conf_LinMNIST_$(KK)NN_$(prop)_eucl_eucl.jld2"; CC)
