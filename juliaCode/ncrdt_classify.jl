function prepare_data(data, num_angles)
    ref = ones(size(data[1,:,:]));
    data1 = rcdt(ref, data[1,:,:], num_angles)[1];
    data_transformed = zeros(size(data)[1], size(data1)[1], size(data1)[2]);
    data_transformed[1,:,:] = data1;
    for i in 2:size(data)[1]
        data_transformed[i,:,:] = rcdt(ref, data[i,:,:], num_angles)[1];
    end
...
function classify_data_NRCDT(samp, image_size, data_size, random_seed, num_angles_rcdt, num_angles_rcdt_norm, num_rcdt)
    ...
    temp_rcdt1 = transpose(rcdt(ref, temp[1,:,:], num_angles_rcdt)[1])
    temp_rcdt = zeros(size(temp)[1], size(temp_rcdt1)[1], size(temp_rcdt1)[2])
    temp_rcdt[1,:,:] = temp_rcdt1
    for i in 2:size(temp)[1]
        temp_rcdt[i,:,:] = transpose(rcdt(ref, temp[i,:,:], num_angles_rcdt)[1])
    end
    #
    data_rcdt1 = transpose(rcdt(ref, dataset[1,:,:], num_angles_rcdt)[1])
    data_rcdt = zeros(size(dataset)[1], size(data_rcdt1)[1], size(data_rcdt1)[2])
    data_rcdt[1,:,:] = data_rcdt1
    for i in 2:size(dataset)[1]
        data_rcdt[i,:,:] = transpose(rcdt(ref, dataset[i,:,:], num_angles_rcdt)[1])
    end
    ...
    for i in 1:num_rcdt
        ...
        train_data_transformed = prepare_data(train_data, i)

        clf_rcdt_model = linear_train(train_labels, transpose(train_data_transformed),  C = 1e10, verbose=false);

        test_data_transformed = prepare_data(test_data, i)
    ...
    train_rcdt1 = transpose(rcdt(ref, train_data[1,:,:], num_angles_rcdt_norm)[1])
    ...
    for i in 2:size(train_data)[1]
        train_rcdt[i,:,:] = transpose(rcdt(ref, train_data[i,:,:], num_angles_rcdt_norm)[1])
    end
    train_rcdt_normalized = (train_rcdt .- mean(train_rcdt, dims=2))./sqrt.(var(train_rcdt, dims=2))
    train_rcdt_normalized = dropdims(maximum(train_rcdt_normalized, dims=3), dims=3)

    clf_rcdt_normalized_model = linear_train(train_labels, transpose(train_rcdt_normalized),  C = 1e10, verbose=false);


    test_rcdt1 = transpose(rcdt(ref, test_data[1,:,:], num_angles_rcdt_norm)[1])
    test_rcdt = zeros(size(test_data)[1], size(test_rcdt1)[1], size(test_rcdt1)[2])
    test_rcdt[1,:,:] = test_rcdt1
    for i in 2:size(test_data)[1]
        test_rcdt[i,:,:] = transpose(rcdt(ref, test_data[i,:,:], num_angles_rcdt_norm)[1])
    end
    test_rcdt_normalized = (test_rcdt .- mean(test_rcdt, dims=2))./sqrt.(var(test_rcdt, dims=2))
    test_rcdt_normalized = dropdims(maximum(test_rcdt_normalized, dims=3), dims=3)


    pred_rcdt_normalized = linear_predict(clf_rcdt_normalized_model, transpose(test_rcdt_normalized))[1];
    acc_rcdt_normalized = mean(test_labels .== pred_rcdt_normalized)

    println("Accuracy of NRCDT : \t", acc_rcdt_normalized)
    println("---------------------------------------------------------------------------------------------")
end;